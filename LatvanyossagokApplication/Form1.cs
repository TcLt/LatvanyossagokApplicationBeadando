...
namespace LatvanyossagokApplication
{
    public partial class Form1 : Form
    {
        ...
        {
            InitializeComponent();
            conn = new MySqlConnection("Server = 127.0.0.1; Database = latvanyossagokdb; Uid = root; Pwd = ;");
            conn.Open();
        ...
        void VarosListazas()
        {
            ...
            cmd.CommandText = @"SELECT id, nev, lakossag FROM varosok ORDER BY id";
        ...
        {
            if (string.IsNullOrWhiteSpace(textBoxVarosNev.Text) || string.IsNullOrWhiteSpace(textBoxLakossag.Text))
            {
        ...
        private void buttonLatvanyossagHozzaad_Click(object sender, EventArgs e)
        {
            ...
            var varos = (Varos)listBoxVaros.SelectedItem;
            if (string.IsNullOrWhiteSpace(textBoxLatvanyossagNev.Text) || string.IsNullOrWhiteSpace(textBoxLeiras.Text) || string.IsNullOrWhiteSpace(textBoxAr.Text))
            {
                MessageBox.Show("Minden mezőt ki kell tölteni!", "Hiba", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return; // void metódus futása megszakad return-el
        ...
        private void buttonVarosTorles_Click(object sender, EventArgs e)
        {
            ...
            if (varos == null)
            {
                MessageBox.Show("Jelöljön ki valamit a törléshez!", "Hiba", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return; // void metódus futása megszakad return-el
